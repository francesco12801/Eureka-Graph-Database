...
//  a. Simple User-based recommendations
MATCH p=(u:User{screenName: "jdfollowhelp"})-[:FOLLOWS*2..3]->(other_u)
WHERE NOT (u)-[:FOLLOWS]->(other_u) AND u <> other_u
RETURN u, other_u, count(other_u) AS frequency, length(p) AS hops
ORDER BY hops ASC, frequency DESC LIMIT 10;

//  b. Simple Item-based reccomentations
MATCH (u:User{screenName: "jdfollowhelp"})-[:TWEETED]->(p)
-[:HAS_TAG]->(t)<-[:HAS_TAG]-(other_p)<-[:TWEETED]-(other_u)
